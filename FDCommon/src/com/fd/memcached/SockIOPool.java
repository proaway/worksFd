 ...
 *  			sock.flush();	
 *  		}
 *  		catch (IOException ioe) { log.info( &quot;io exception thrown&quot; ) };	
 *  
 *  		sock.close();	
 * </pre>
 * 
				...
				if (aSockets != null && !aSockets.isEmpty())
				{
							...
							.hasNext();)
					{
						...
						} else
						{
							...
							// log.error("++++ socket in avail pool is not
							// connected: "
							// + socket.toString()
							// + " for host: "
							// + host);
		...
		synchronized (this)
		{
					...
					.hasNext();)
			{
				...
				} else if (sockets.size() > maxConn)
				{
							...
							.hasNext();)
					{
						...
						{
							// log
							// .debug("+++ removing stale entry from pool as it
							// is past its idle timeout and pool is over max
							// spare");
							try
							{
								socket.trueClose();
			...
			for (Iterator<String> i = busyPool.keySet().iterator(); i.hasNext();)
			{
						...
						.hasNext();)
				{
					...
					{
						// log
						// .error("+++ removing potentially hung connection from
						// busy pool ... socket in pool for "
