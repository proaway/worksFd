 ...
 * @version:  V1.0
 */
public class AjaxProductImageUpload extends SecureAction {
	...
	public void doPerform(RunData data, Context context) throws Exception {
		...
		pib.setIndexId(0);
		productManager.insertProductImage(pib);
	}
...
/*		IProductUploadManager uploadProductManager = (IProductUploadManager)AppContextUtil.getBean("productUploadManager");
			...
			for(int xx=0;xx<mainProductConfigs.size();xx++){
				...
				if( subProductConfigs != null && subProductConfigs.size() > 0){
					for(int yy=0;yy<subProductConfigs.size();yy++){
						...
						String sku = data.getParameters().getString("sku_"+valueAttrId+"_"+subValueAttrId);
					...
					String sku = data.getParameters().getString("sku_"+valueAttrId+"_");
		...
		if(ifDiscount != null && "1".endsWith(ifDiscount)){
			...
			priceBean.setDiscountStartDate(sdf.parse(discountEndDate));
		}
		...
		if (productId > 0) { // 更新
			...
			p.setSpecifications(data.getParameters().getString("specificationsUrl"));
