 ...
 * 
 */
public class AjaxSaveProductSize extends SecureAction {
	@Override
...
//		double addWeight = data.getParameters().getDouble("addWeight");
		int length = data.getParameters().getInt("length");
		int width = data.getParameters().getInt("width");
		int height = data.getParameters().getInt("height");
		
		IProductManager productManager = (IProductManager) getBean("productManager");
		int i = productManager.updateProductSize(productIds, length, width, height);
