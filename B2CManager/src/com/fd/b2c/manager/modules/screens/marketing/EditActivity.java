 ...
 * @version:  V1.0
 */
public class EditActivity extends SecureScreen {
	@Override
	public void doBuildTemplate(RunData data, Context context) throws Exception {
		...
		String countrys = ab.getCountry();
		if(countrys != null){
			List<RegionBean> rbs = new ArrayList<RegionBean>();
			String[] country = countrys.split(",");
			for(int i=0;i<country.length;i++){
				RegionBean rb = new RegionBean();
				rb = dictManager.getRegionByRegionId(country[i]);
				rbs.add(rb);
			}
			context.put("regions", rbs);
		}
		if(ab.getType() == 0){
