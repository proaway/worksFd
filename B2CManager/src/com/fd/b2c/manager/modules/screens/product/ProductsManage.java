 ...
 * @version:  V1.0
 */
public class ProductsManage extends SecureScreen{
	public void doBuildTemplate(RunData data, Context context) throws Exception {
		...
		int productStatuses[] = data.getParameters().getInts("productStatuses");
		
		if (data.getParameters().get("orderType") == null) {
			searchVo.setOrderType("2");
			if (productStatuses == null) {
				searchVo.addProductStatus(ProductConstants.ONSALE);
				searchVo.addProductStatus(ProductConstants.DRAFT);
				searchVo.addProductStatus(ProductConstants.IMPORT);
				searchVo.addProductStatus(ProductConstants.OFFSALE);
			}
		}
		if (productStatuses != null) {
			for (int productStatus : productStatuses) {
				searchVo.addProductStatus(productStatus);
			}
		}
